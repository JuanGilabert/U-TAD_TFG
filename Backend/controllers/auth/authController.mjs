...
export class AuthController {
    ...
    postNewUser = async (req, res) => {
        ...
        if (registerUserModelResponse?.message === "Existing email.")
            return res.status(409).send({ message: `${CONFLICT_409_MESSAGE}. ${registerUserModelResponse.message}` });
        if (registerUserModelResponse?.message === "Error creating user.")
            return res.status(500).send({ message: `${INTERNAL_SERVER_ERROR_500_MESSAGE}. ${registerUserModelResponse.message}.` });
