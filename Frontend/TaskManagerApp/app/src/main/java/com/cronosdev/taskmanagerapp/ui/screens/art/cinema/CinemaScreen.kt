...
fun CinemaScreen(navController: NavHostController, cinemaViewModel: CinemaViewModel) {
    ...
    LaunchedEffect(Unit) { cinemaViewModel.fetchAvailableCinemas(Destinations.ART_CINEMA_SCREEN_URL) }
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        ...
        Column(horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Top,
            ...
            if (cinemaViewModel.showSearchedCinemas) {
                if (cinemaViewModel.matchRoomList.isResultListNullOrEmpty()) {
                    ...
                    cinemaViewModel.matchRoomList.data?.let {
                        ShowCinemaList(it) { id ->
                            ...
                            navController.navigate(Destinations.ART_EDIT_CINEMA_SCREEN_URL)
                        }
                    }
                }
            }
            else cinemaViewModel.availableCinemasList.data?.let {
                ShowCinemaList(it) { id ->
                    cinemaViewModel.fetchSelectedCinemaData(id);
                    navController.navigate(Destinations.ART_EDIT_CINEMA_SCREEN_URL)
                }
