...
fun EditCinemaScreen(navController: NavHostController, cinemaViewModel: CinemaViewModel) {
    ...
    LaunchedEffect(Unit) {
        cinemaViewModel.fetchUnavailableDates(Destinations.ART_EDIT_CINEMA_SCREEN_URL)
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        ...
        Column(modifier = Modifier.fillMaxSize().padding(10.dp)/*.border(width = 2.dp, color = Color.Red)*/,
            ...
            ShowUpdateScreenButtons(onButtonClick = { userAction ->
                ...
                if (userAction == "save") cinemaViewModel.putCinema(Destinations.ART_EDIT_CINEMA_SCREEN_URL)
                if (userAction == "delete") cinemaViewModel.deleteCinema(Destinations.ART_EDIT_CINEMA_SCREEN_URL)
            })
        }
    }
}
