...
class FoodViewModel @Inject constructor(private val foodRepository: FoodRepository): ViewModel()  {
    ...
    //
    private val _editFoodScreenUiState = MutableStateFlow<UiState?>(null)
    val editFoodScreenUiState: StateFlow<UiState?> = _editFoodScreenUiState
    //
    ...
    private val _exceptionHandler = CoroutineExceptionHandler { context, throwable ->
        ...
        if (screenCaller == Destinations.EDIT_FOOD_SCREEN_URL) fetchEditFoodScreenUIState(UiState.Error("throwableErrorType", screenCaller, methodCaller, errorMessage))
    }
    ...
    private fun fetchEditFoodScreenUIState(state: UiState) { _editFoodScreenUiState.value = state }
    ...
    //
    private fun _setFoodVariables() {_id = ""
    ...
    private fun createFoodRequest(): FoodModel {
        return FoodModel(_id = "", _nombreRestaurante, _direccionRestaurante, _tipoComida, _fechaReserva, _asistentesReserva, _notasReserva)
    }
