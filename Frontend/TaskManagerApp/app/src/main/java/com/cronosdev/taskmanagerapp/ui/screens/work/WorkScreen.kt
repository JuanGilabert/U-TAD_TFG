...
fun WorkScreen(navController: NavHostController, workViewModel: WorkViewModel) {
    ...
    LaunchedEffect(Unit) { workViewModel.fetchAvailableWorks(Destinations.WORK_SCREEN_URL) }
...
fun ShowWorkScreenSuccessUiState(uiState: UiState.Success, navController: NavHostController, workViewModel: WorkViewModel) {
    ...
    if ((uiState).successType == "screenRunning") {
        ...
        if ((uiState).successScreenCallName == Destinations.EDIT_WORK_SCREEN_URL) {
            if ((uiState).successMethodName == "putWork") navController.popBackStack(Destinations.TRAVEL_SCREEN_URL, false)
            if ((uiState).successMethodName == "deleteWork") navController.popBackStack(Destinations.TRAVEL_SCREEN_URL, false)
        }
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        ...
        Column(horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Top,
            ...
            ShowTextFieldSearcherComponent(
                onValueSearcherChange = { workViewModel.fetchPaintingToSearch(it) },
            ...
            if (workViewModel.showSearchedWorks) workViewModel.matchWorksList.data?.let { ShowWorkList(it,
                onCardClick = { id -> workViewModel.fetchSelectedPaintingData(id); navController.navigate(Destinations.EDIT_WORK_SCREEN_URL) })
            }
