...
fun ShowMenu(onMenuItemClick: (String) -> Unit, authViewModel: AuthViewModel) {
    ...
    val signOutResult by authViewModel.signoutResultUiState.collectAsState()
    ...
    when (signOutResult) {
        ...
        is UiState.Success -> {
            (context as MainActivity).apply {
                showToastMessage(context, authViewModel.logOutApiResponse.message)
                // Iniciamos el AuthActivity y finalizamos el actual(MainActivity).
                startActivity(intent)
                finish()
            }
        }
        is UiState.Error -> {
            if ((signOutResult as UiState.Error).errorType == "throwableErrorType") {
                if ((signOutResult as UiState.Error).errorScreenCallName == "drawerContent") {
                    if ((signOutResult as UiState.Error).errorMethodName == "signout") {
                        AlertDialog(
                            //title = { Text(alertDialogMessageToShow) },
                            text = { Text((signOutResult as UiState.Error).errorMessage) },
                            onDismissRequest = {
                                authViewModel.fetchSignoutResultUIState(null)
                                onMenuItemClick(Destinations.ALL_TASKS_SCREEN_URL); },
                            confirmButton = { TextButton(onClick = { authViewModel.signout() }) { Text("Reintentar") } },
                            dismissButton = { TextButton(onClick = { onMenuItemClick(Destinations.ALL_TASKS_SCREEN_URL) }) { Text("No") } }
                        )
                    }
                }
            }
            if ((signOutResult as UiState.Error).errorType == "fetchDataErrorType") {
                if ((signOutResult as UiState.Error).errorScreenCallName == "drawerContent") {
                    if ((signOutResult as UiState.Error).errorMethodName == "signout")
                        (context as MainActivity).apply {
                            showToastMessage(context, authViewModel.logOutApiResponse.message)
    ...
    // Item allTasksScreen
    ShowMenuItem(R.drawable.music_note_24dp_1e90ff_fill0_wght400_grad0_opsz24, stringResource(R.string.taskScreenViewName), onMenuItemTextClick = { onMenuItemClick(Destinations.ALL_TASKS_SCREEN_URL)} )
    // Item cinemaScreen
    ShowMenuItem(R.drawable.artcinemadrawercontenticon, stringResource(R.string.cinemaScreenViewName), onMenuItemTextClick = { onMenuItemClick(Destinations.ART_CINEMA_SCREEN_URL) })
    // Item musicScreen
    ShowMenuItem(R.drawable.artmusicdrawercontenticon, stringResource(R.string.musicScreenViewName), onMenuItemTextClick = { onMenuItemClick(Destinations.ART_MUSIC_SCREEN_URL) })
