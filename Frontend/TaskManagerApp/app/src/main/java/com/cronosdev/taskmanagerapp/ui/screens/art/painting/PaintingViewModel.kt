...
class PaintingViewModel @Inject constructor(private val paintingRepository: PaintingRepository): ViewModel()  {
    ...
    //
    private val _editPaintingScreenUiState = MutableStateFlow<UiState?>(null)
    val editPaintingScreenUiState: StateFlow<UiState?> = _editPaintingScreenUiState
    //
    ...
    private val _exceptionHandler = CoroutineExceptionHandler { context, throwable ->
        ...
        if (screenCaller == Destinations.ART_EDIT_PAINTING_SCREEN_URL)
    ...
    private fun fetchEditPaintingScreenUIState(state: UiState) { _editPaintingScreenUiState.value = state }
    ...
    //
    private fun _setPaintingVariables() {
