...
fun UpdateMusicScreen(navController: NavHostController, musicViewModel: MusicViewModel) {
    ...
    val uiState by musicViewModel.updateMusicScreenUiState.collectAsState()
    // Conjunto de acciones que se realizan al cargar la vista, en este caso obtener la lista de salas disponibles para reservar.
    LaunchedEffect(Unit) { musicViewModel.fetchUnavailableDates(Destinations.ART_UPDATE_MUSIC_SCREEN_URL) }
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        ...
        Column(modifier = Modifier.fillMaxSize().padding(10.dp)/*.border(width = 2.dp, color = Color.Red)*/,
            ...
            ShowUpdateScreenButtons(onButtonClick = { userAction ->
                if (userAction == "back") navController.popBackStack(Destinations.ART_MUSIC_SCREEN_URL, false)
                if (userAction == "save") musicViewModel.putMusic(Destinations.ART_UPDATE_MUSIC_SCREEN_URL)
                if (userAction == "delete") musicViewModel.deleteMusic(Destinations.ART_UPDATE_MUSIC_SCREEN_URL)
            })
        }
    }
}
