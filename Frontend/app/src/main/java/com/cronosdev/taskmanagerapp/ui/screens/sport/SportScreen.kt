...
fun SportScreen (navController: NavHostController, sportViewModel: SportViewModel) {
    ...
    LaunchedEffect(Unit) {
        sportViewModel.setSportVariables()
        sportViewModel.setShowSearchedSports(false)
        sportViewModel.fetchAvailableSports(Destinations.SPORT_SCREEN_URL)
    }
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        ...
        Column(horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Top,
            ...
            if (sportViewModel.showSearchedSports) sportViewModel.matchSportsList.data?.let { ShowPaintingList(it,
                onCardClick = { id -> sportViewModel.fetchSelectedSportData(id); navController.navigate(Destinations.EDIT_SPORT_SCREEN_URL) })
            }
