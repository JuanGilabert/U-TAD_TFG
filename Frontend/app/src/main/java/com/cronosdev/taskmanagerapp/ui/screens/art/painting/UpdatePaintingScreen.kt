...
fun UpdatePaintingScreen(navController: NavHostController, paintingViewModel: PaintingViewModel) {
    ...
    val uiState by paintingViewModel.updatePaintingScreenUiState.collectAsState()
    // Conjunto de acciones que se realizan al cargar la vista, en este caso obtener la lista de salas disponibles para reservar.
    LaunchedEffect(Unit) { paintingViewModel.fetchUnavailableDates(Destinations.ART_UPDATE_PAINTING_SCREEN_URL) }
...
fun ShowUpdatePaintingScreen(navController: NavHostController, paintingViewModel: PaintingViewModel) {
    Box(modifier = Modifier.fillMaxSize()) {
        Image(painter = painterResource(id = R.drawable.bckpaintingblack), contentDescription = null,
        ...
        Column(modifier = Modifier.fillMaxSize().padding(10.dp)/*.border(width = 2.dp, color = Color.Red)*/,
            ...
            ShowUpdateScreenButtons(onButtonClick = { userAction ->
                if (userAction == "back") navController.popBackStack(Destinations.ART_PAINTING_SCREEN_URL, false)
                if (userAction == "save") paintingViewModel.putPainting(Destinations.ART_UPDATE_PAINTING_SCREEN_URL)
                if (userAction == "delete") paintingViewModel.deletePainting(Destinations.ART_UPDATE_PAINTING_SCREEN_URL)
            })
        }
    }
}
