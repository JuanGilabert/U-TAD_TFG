...
class WorkViewModel @Inject constructor(private val workRepository: WorkRepository): ViewModel()  {
    ...
    //
    private val _updateWorkScreenUiState = MutableStateFlow<UiState?>(null)
    val updateWorkScreenUiState: StateFlow<UiState?> = _updateWorkScreenUiState
    //
    private val _exceptionHandler = CoroutineExceptionHandler { context, throwable ->
        ...
        if (screenCaller == Destinations.WORK_SCREEN_URL)
            fetchWorkScreenUIState(UiState.Error("throwableErrorType", screenCaller, methodCaller, errorMessage))
        if (screenCaller == Destinations.CREATE_WORK_SCREEN_URL)
            fetchCreateWorkScreenUIState(UiState.Error("throwableErrorType", screenCaller, methodCaller, errorMessage))
        if (screenCaller == Destinations.EDIT_WORK_SCREEN_URL)
            fetchEditWorkScreenUIState(UiState.Error("throwableErrorType", screenCaller, methodCaller, errorMessage))
    }
    ...
    private fun fetchEditWorkScreenUIState(state: UiState) { _updateWorkScreenUiState.value = state }
    ...
    // Set all values
    fun setWorkVariables() { _id = ""; setTituloTarea(""); setDescripcionTarea("");
    ...
    // Fetch Work Data To Update On Screen
    fun fetchSelectedWorkData(workId: String) {
    ...
    fun setShowSearchedWorks(value: Boolean) { _showSearchedWorks = value }
    //
     ...
     */
    fun fetchWorkToSearch(nameToSearch: String) {
