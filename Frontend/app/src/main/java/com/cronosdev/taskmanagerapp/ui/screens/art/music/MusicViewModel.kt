...
class MusicViewModel @Inject constructor(private val musicRepository: MusicRepository): ViewModel()  {
    ...
    //
    private val _updateMusicScreenUiState = MutableStateFlow<UiState?>(null)
    val updateMusicScreenUiState: StateFlow<UiState?> = _updateMusicScreenUiState
    //
    ...
    private val _exceptionHandler = CoroutineExceptionHandler { context, throwable ->
        ...
        if (screenCaller == Destinations.ART_UPDATE_MUSIC_SCREEN_URL)
    ...
    private fun fetchEditMusicScreenUIState(state: UiState) { _updateMusicScreenUiState.value = state }
    ...
    // Fetch Music Data To Update On Screeen
    ...
    fun setShowSearchedMusics(value: Boolean) { _showSearchedMusics = value }
    //
